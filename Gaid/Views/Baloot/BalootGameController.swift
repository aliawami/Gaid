...
final class BalootGameController: ObservableObject, GamePlay{
    ...
    func newGame(){
        id += 1
        self.namingPlayers(names: groups, groupsNames: [])
        shufflerID = 0
...
//            player.scores = Array.init(repeating: -1, count: 10)
//        }
    }
    
    func regsiterScore(scors: [Int]){
        for index in 0 ..< players.count{
            players[index].scores.append(scors[index])
        }
...
extension BalootGameController: ShfflingDelegate{
...
//Scores Monitoring
extension BalootGameController{
